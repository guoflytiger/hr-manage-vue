<template>
  <div v-loading="globalLoading"
    ...
    <el-card class="box-card">
      ...
      <div class="permissManaMain" style="text-indent: 15px;font-weight: 1000;">
        <el-collapse v-model="activeName"
          ...
          accordion
          @change="change">
          <el-collapse-item style="font-weight: 700;" :title="r.nameZh" :name="r.id" v-for="(r,index) in roles" :key="index">
            <el-card class="box-card">
              <div slot="header" class="clearfix">
                <span>可访问的资源</span>
...
export default {
  ...
  methods: {
    initRoles() {
      this.loading = true;
      this.getRequest("/per/permiss/allRoles").then(resp => {
        this.loading = false;
        if (resp) {
          this.roles = resp;
        }
    ...
    initAllMenus() {
      this.getRequest("/per/permiss/menus").then(resp => {
        if (resp) {
          this.allmenus = resp;
        }
      })
    ...
    initSelectedMenus(rid) {
      this.getRequest("/per/permiss/mids/" + rid).then(resp => {
        if (resp) {
    ...
    doAddRole() {
      if (this.role.name && this.role.nameZh) {
        ...
        this.postRequest("/per/permiss/addRole", this.role).then(resp => {
    ...
    deleteRole(role) {
      this.$confirm('此操作将永久删除【' + role.nameZh + '】角色, 是否继续?', '提示', {
        ...
        this.deleteRequest("/per/permiss/role/" + role.id).then(resp => {
          if (resp) {
            this.initRoles();
          }
        })
